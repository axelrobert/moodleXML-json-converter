...
public class JSONSaxAdapter {
	...
	public JSONSaxAdapter(final JsonParser jsonParser,
			final ContentHandler contentHandler) {
		this(jsonParser, contentHandler, "");
	}

	public JSONSaxAdapter(final JsonParser jsonParser,
			final ContentHandler contentHandler, final String namespaceUri) {
		this(jsonParser, contentHandler, namespaceUri, false);
	}

	public JSONSaxAdapter(final JsonParser jsonParser,
			final ContentHandler contentHandler, final String namespaceUri,
			final boolean addTypeAttributes) {
		...
		try {
			...
			if (elementsWritten > 1) {
				throw new ParserException(
						"More than one root element. Can not generate legal XML.");
			}
	...
	private int parseObject() throws IOException, JsonParseException, Exception {
		...
		while (jsonParser.nextToken() != null
			...
			if (FIELD_NAME.equals(jsonParser.getCurrentToken())) {
				...
				throw new ParserException(
						"Error when parsing. Expected field name got "
	...
	private void startElement(final String elementName) throws SAXException {
		contentHandler.startElement(namespaceUri, elementName, elementName,
				getTypeAttributes());
	}
	...
	protected Attributes getTypeAttributes() {
		if (addTypeAttributes) {
			...
			if (currentTokenType != null) {
				...
				attributes.addAttribute("", "type", "type", "string",
						currentTokenType);
				return attributes;
			} else {
	...
	private class DocumentLocator implements Locator {
		...
		public int getLineNumber() {
			return jsonParser.getCurrentLocation() != null ? jsonParser
					.getCurrentLocation().getLineNr() : -1;
		}

		public int getColumnNumber() {
			return jsonParser.getCurrentLocation() != null ? jsonParser
					.getCurrentLocation().getColumnNr() : -1;
		}
	}
}