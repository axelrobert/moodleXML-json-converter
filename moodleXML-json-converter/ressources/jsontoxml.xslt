<xsl:stylesheet version="1.0"
	...
	xmlns:json="http://www.ibm.com/xmlns/prod/2009/jsonx" xmlns:exsl="http://exslt.org/common"
	...
	<xsl:template match="/*">
		...
		<xsl:apply-templates select="exsl:node-set($t1)/so:output/*"
			mode="copy-sans-namespace" />
	</xsl:template>
	...
	<xsl:template name="field">
		...
		<xsl:variable name="field-name"
			select="substring-before(substring-after($json-in,$quot),$quot)" />
	...
	<xsl:template name="fields">
		...
		<!-- output like: <so:output><Open>25.15</Open><High>25.15</High></so:output> 
			<so:extra>} bla</so:extra> -->
		<xsl:param name="json-in" />
		...
		<xsl:choose>
			<xsl:when test="substring($n,1,1) = $quot">
				...
				<xsl:variable name="t2"
					select="normalize-space( exsl:node-set($t1)/so:extra) " />
				...
				<so:output>
					<xsl:copy-of
						select="exsl:node-set($t1)/so:output/* | exsl:node-set($t3)/so:output/*" />
				</so:output>
	...
	<xsl:template name="object">
		...
		<xsl:variable name="t1"
			select="normalize-space(substring-after($json-in,'{'))" />
		<xsl:variable name="t2">
			<xsl:call-template name="fields">
		...
		<xsl:variable name="t3"
			select="normalize-space(substring-after( exsl:node-set($t2)/so:extra, '}'))" />
		<so:output>
			<xsl:choose>
	...
	<xsl:template name="value">
		...
		<xsl:variable name="first-letter"
			select="substring(normalize-space($json-in),1,1)" />
		<xsl:choose>
	...
	<xsl:template name="string">
		...
		<xsl:variable name="remainder"
			select="normalize-space(substring-after(substring-after($json-in,$quot),$quot))" />
		<so:output>
			<xsl:element name="{$parent-ele}">
